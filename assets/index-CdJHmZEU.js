var q=Object.defineProperty;var N=(n,e,t)=>e in n?q(n,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[e]=t;var r=(n,e,t)=>N(n,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();const w="6e400001-b5a3-f393-e0a9-e50e24dcca9e",R="6e400002-b5a3-f393-e0a9-e50e24dcca9e",I="6e400003-b5a3-f393-e0a9-e50e24dcca9e",S=200,E=3e3;class P{constructor(){r(this,"device",null);r(this,"server",null);r(this,"rxChar",null);r(this,"buffer","");r(this,"queue",[]);r(this,"notifyWaiters",[]);r(this,"_connected",!1);r(this,"opChain",Promise.resolve())}get connected(){return this._connected}drainQueue(){const e=[...this.queue];return this.queue=[],e}async connect(){var i;if(!navigator.bluetooth)throw new Error("Web Bluetooth is not available in this browser.");this.device=await navigator.bluetooth.requestDevice({filters:[{services:[w]}],optionalServices:[w]}),this.device.addEventListener("gattserverdisconnected",()=>{this._connected=!1});const e=await((i=this.device.gatt)==null?void 0:i.connect());if(!e)throw new Error("GATT connect failed");this.server=e;const t=await this.server.getPrimaryService(w);this.rxChar=await t.getCharacteristic(R);const s=await t.getCharacteristic(I);await s.startNotifications(),s.addEventListener("characteristicvaluechanged",a=>{const c=a.target.value;if(!c)return;const f=new TextDecoder().decode(c);for(this.buffer+=f;this.buffer.includes(`
`);){const m=this.buffer.indexOf(`
`),p=this.buffer.slice(0,m).trim();if(this.buffer=this.buffer.slice(m+1),!p)continue;const y=this.notifyWaiters.shift();y?y(p):this.queue.push(p)}}),this._connected=!0}async withLock(e){const t=this.opChain.then(()=>e());return this.opChain=t.then(()=>{},()=>{}),t}async writeLine(e){if(!this.rxChar)throw new Error("Not connected");const t=new TextEncoder().encode(e+`
`);await this.rxChar.writeValueWithoutResponse(t)}async waitForLine(e){return this.queue.length?this.queue.shift():new Promise(t=>{const s=window.setTimeout(()=>{const a=this.notifyWaiters.indexOf(i);a>=0&&this.notifyWaiters.splice(a,1),t(null)},e),i=a=>{clearTimeout(s),t(a)};this.notifyWaiters.push(i)})}async send(e){return this.withLock(async()=>e?(await this.writeLine(e),this.collectLines(S,E)):this.collectLines(S,E))}async sendAndCollect(e,t=S,s=E,i){return this.withLock(async()=>(e&&await this.writeLine(e),this.collectLines(t,s,i)))}async collectLines(e,t,s){const i=[],a=Date.now()+t;for(;Date.now()<a;){const l=a-Date.now(),c=await this.waitForLine(Math.min(e,l));if(c===null){if(i.length)return i;continue}if(i.push(c),s!=null&&s(c,i))return i}return i}disconnect(){var e;(e=this.server)==null||e.disconnect(),this._connected=!1,this.device=null,this.server=null,this.rxChar=null,this.queue=[],this.notifyWaiters=[],this.opChain=Promise.resolve()}}const W=250,F=8e3,U=5e3,G=200;function B(n){const e=`OK ${n}`;return t=>t===e||t.startsWith(`${e} `)}function K(){const n=new Date(new Date().getFullYear(),0,1),e=new Date(new Date().getFullYear(),6,1),t=n.getTimezoneOffset(),s=e.getTimezoneOffset(),i=-t*60,l=t!==s?-Math.min(t,s)*60:0;return`TZ_OFFSET ${i} ${l}`}function Y(){const n=new Date,e=t=>String(t).padStart(2,"0");return`TIME ${n.getFullYear()}-${e(n.getMonth()+1)}-${e(n.getDate())} ${e(n.getHours())}:${e(n.getMinutes())}:${e(n.getSeconds())}`}function g(n){return n==="REQ_TIME"||n.startsWith("REQ_TIME ")}async function z(n){let e=n.drainQueue().some(g);if((await n.send(K())).some(g)&&(e=!0),!e){const s=Date.now()+2e3;for(;Date.now()<s;){const i=Math.max(100,Math.min(G,s-Date.now())),a=await n.sendAndCollect("",i,Math.min(600,s-Date.now()),l=>g(l));if(a.some(g)){e=!0;break}if(!a.length)break}}}async function j(n){await n.send(Y())}async function L(n){return await j(n),J(n)}async function k(n){const e=await n.sendAndCollect("SCHED_GET",W,F,B("SCHED_GET"));if(!e.some(t=>t.startsWith("OK SCHED_GET")))throw new Error("SCHED_GET failed");return e}async function V(n,e){const t=Object.keys(e);for(const i of t){const a=e[i];let l;a.enabled?l=`SCHED_DAY ${i} ${a.time} ${a.prewindowMin} ${a.pwmMax} ${a.postHoldMin}`:l=`SCHED_DAY ${i} OFF`;const c=await n.send(l);if(!c.some(f=>f.startsWith("OK SCHED_DAY")))throw new Error(`Failed to set ${i}: ${c.join(" ")}`)}if(!(await n.send("SAVE")).some(i=>i.includes("SAVE ok")))throw new Error("SAVE failed")}function X(n){const e={};for(const t of n)t.startsWith("RTC: ")&&(e.rtc=t.slice(5)),t.startsWith("TODAY: ")&&(e.today=t.slice(7).trim()),t.startsWith("TODAY_ALARM: ")&&(e.todayAlarm=t.slice(13)),t.startsWith("ALARM(daily): ")&&(e.alarmDaily=t.slice(14)),t.startsWith("ALARM_AT: ")&&(e.alarmAt=t.slice(10)),t.startsWith("NEXT_RAMP: ")&&(e.nextRamp=t.slice(11)),t.startsWith("WAKE_CAUSE: ")&&(e.wakeCause=t.slice(12)),t.startsWith("BOOT_PATH: ")&&(e.bootPath=t.slice(11)),t.startsWith("NVM_OK: ")&&(e.nvmOk=t.includes("yes")),t.startsWith("PHASE: ")&&(e.phase=t.slice(7)),t.startsWith("TIME_SYNC_AUTO: ")&&(e.timeSync=t.slice(16)),t.startsWith("TIME_TRUSTED: ")&&(e.timeTrusted=t.includes("yes")),t.startsWith("PRE(min): ")&&(e.preMin=t.slice(10)),t.startsWith("POST_HOLD(min): ")&&(e.postHoldMin=t.slice(16));return e}async function J(n){const e=await n.sendAndCollect("STATUS_LITE",W,U,B("STATUS_LITE"));if(e.some(t=>t.includes("ERR unknown cmd: STATUS_LITE")))return{};if(!e.some(t=>t.startsWith("OK STATUS_LITE")))throw new Error("STATUS_LITE failed");return X(e)}function Q(){return typeof navigator<"u"&&!!navigator.bluetooth}function Z(){return/iPad|iPhone|iPod/.test(navigator.userAgent)}function ee(){if(!Z())return!1;const n=navigator.userAgent;return/Safari/i.test(n)&&!/CriOS|FxiOS|EdgiOS/i.test(n)}const h=["mon","tue","wed","thu","fri","sat","sun"],H="sml-draft-schedule-v2";function te(){return{enabled:!0,time:"07:00",prewindowMin:20,pwmMax:255,postHoldMin:20}}function T(){return Object.fromEntries(h.map(n=>[n,te()]))}function se(){try{const n=localStorage.getItem(H);if(n){const t=JSON.parse(n);return C(t)}const e=localStorage.getItem("sml-draft-schedule-v1");if(e){const t=JSON.parse(e);return C(t)}return null}catch{return null}}function C(n){const e=T();for(const t of h){const s=n[t];s&&(e[t]={enabled:s.enabled,time:s.time,prewindowMin:s.prewindowMin??20,pwmMax:s.pwmMax??255,postHoldMin:s.postHoldMin??20})}return e}function u(n){localStorage.setItem(H,JSON.stringify(n))}function M(n){const e=T();for(const t of n){const s=t.match(/^DAY\s+(mon|tue|wed|thu|fri|sat|sun)\s+(ON|OFF)\s+(\d{2}:\d{2})\s+(\d+)\s+(\d+)(?:\s+(\d+))?/i);if(!s)continue;const i=s[1].toLowerCase();e[i]={enabled:s[2].toUpperCase()==="ON",time:s[3],prewindowMin:Number(s[4]),pwmMax:Number(s[5]),postHoldMin:s[6]?Number(s[6]):20}}return e}function ie(n,e){return h.every(t=>JSON.stringify(n[t])===JSON.stringify(e[t]))}class ne{constructor(e){r(this,"root");r(this,"screen","connect");r(this,"transport",null);r(this,"schedule",se()??T());r(this,"selected",new Set);r(this,"status",{});r(this,"message","");r(this,"messageKind","");r(this,"busy",!1);r(this,"scheduleLoading",!1);r(this,"statusLoading",!1);r(this,"syncGeneration",0);this.root=e,this.render()}setMessage(e,t=""){this.message=e,this.messageKind=t,this.render()}async withBusy(e){this.busy=!0,this.render();try{await e()}catch(t){const s=t instanceof Error?t.message:String(t);this.setMessage(s,"error")}finally{this.busy=!1,this.render()}}async connectBle(){await this.withBusy(async()=>{const e=new P;await e.connect(),this.transport=e,this.screen="editor",this.status={},this.scheduleLoading=!0,this.statusLoading=!0,this.setMessage("Connected — loading schedule in background…","ok"),this.loadDeviceData(e,++this.syncGeneration)})}async loadDeviceData(e,t){try{if(await z(e),t!==this.syncGeneration)return;const s=await k(e);if(t!==this.syncGeneration)return;const i=await L(e);if(t!==this.syncGeneration)return;this.schedule=M(s),u(this.schedule),this.status=i,this.setMessage("Schedule loaded from mask.","ok")}catch(s){if(t!==this.syncGeneration)return;const i=s instanceof Error?s.message:String(s);this.setMessage(`Sync failed: ${i}`,"error")}finally{t===this.syncGeneration&&(this.scheduleLoading=!1,this.statusLoading=!1,this.render())}}disconnect(){var e;this.syncGeneration++,(e=this.transport)==null||e.disconnect(),this.transport=null,this.screen="connect",this.selected.clear(),this.status={},this.scheduleLoading=!1,this.statusLoading=!1,this.setMessage("")}toggleDay(e){this.schedule[e].enabled=!this.schedule[e].enabled,u(this.schedule),this.render()}patchDay(e,t){Object.assign(this.schedule[e],t),u(this.schedule)}toggleSelect(e){this.selected.has(e)?this.selected.delete(e):this.selected.add(e),this.render()}applyBulk(e,t,s,i,a){const l=this.selected.size?[...this.selected]:h;for(const c of l)e==="time"&&t&&(this.schedule[c].time=t,this.schedule[c].enabled=!0),(e==="brightness"||e==="all")&&s!==void 0&&i!==void 0&&(this.schedule[c].prewindowMin=s,this.schedule[c].pwmMax=i,a!==void 0&&(this.schedule[c].postHoldMin=a)),e==="all"&&t&&(this.schedule[c].time=t,this.schedule[c].enabled=!0);u(this.schedule),this.render()}async saveToDevice(){if(!this.transport)return;if(!h.some(t=>this.schedule[t].enabled)){this.setMessage("Enable at least one day before saving.","error");return}await this.withBusy(async()=>{await V(this.transport,this.schedule);const t=await k(this.transport),s=M(t);if(!ie(this.schedule,s))throw new Error("Device schedule does not match what was sent.");this.status=await L(this.transport),this.setMessage("Schedule saved to mask.","ok")})}async refreshFromDevice(){this.transport&&await this.withBusy(async()=>{const e=await k(this.transport);this.schedule=M(e),u(this.schedule),this.status=await L(this.transport),this.setMessage("Reloaded from mask.","ok")})}renderConnect(){const e=Q(),t=ee(),s=window.location.href;return`
